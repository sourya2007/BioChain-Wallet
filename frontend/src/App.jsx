...
function DNAVerificationScreen({ user, onVerified }) {
  ...
  const handleDNA = async () => {
    ...
    try {
      ...
      addLog("Profile created: " + profile.profile_id.slice(0, 8) + "...", "ok");

      setTimeout(() => setStage("geo"), 600);
    } catch (err) {
      addLog("DNA/profile step failed: " + err.message, "err");
    }
  };
  ...
  const handleGeo = async () => {
    ...
    try {
      addLog("Initializing geo-genomic trust engine...");
      await new Promise(r => setTimeout(r, 250));
      setProgress(15);

      addLog(`Creating currency wallet at location ${geoData.lat}, ${geoData.lon}...`);
      const wallet = await apiRequest("/api/wallets/currency", "POST", {
        profile_id: profileData.profile_id,
        currency: "BTC",
        home_latitude: Number(geoData.lat),
        home_longitude: Number(geoData.lon),
        device_fingerprint: geoData.device,
      });
      setWalletData(wallet);
      setProgress(40);
      addLog("Wallet created: " + wallet.wallet_id.slice(0, 8) + "...", "ok");

      addLog("Connecting wallet session token...");
      const connect = await apiRequest("/api/wallets/connect", "POST", {
        profile_id: profileData.profile_id,
        wallet_id: wallet.wallet_id,
        dna_commitment: dnaHash,
      });
      if (connect.connected && connect.connection_token) {
        setSessionToken(connect.connection_token);
      }
      setProgress(60);
      addLog(connect.message, connect.connected ? "ok" : "err");

      addLog("Evaluating backend trust policy...");
      const trust = await apiRequest("/api/trust/evaluate", "POST", {
        wallet_id: wallet.wallet_id,
      ...
      addLog("Verifying ZK DNA proof hook...");
      const zk = await apiRequest("/api/dna/verify-zk", "POST", {
        wallet_id: wallet.wallet_id,
        proof: `proof_${dnaHash.slice(0, 20)}`,
        public_signal: `signal_${dnaHash.slice(-20)}`,
      });

      const uiTrust = {
        score: Math.max(0, Math.min(1, Number(trust.trust_score) || 0)),
        zone: String(trust.zone || "unknown").toUpperCase(),
        action: trust.action || "full_access",
        delay: trust.delay_seconds || 0,
      };
      setTrustResult(uiTrust);
      setProgress(100);

      addLog(`ZK verifier result: ${zk.valid ? "valid" : "invalid"}`, zk.valid ? "ok" : "err");
      addLog("TRUST ZONE VERIFIED — ACCESS PIPELINE COMPLETE", "ok");
      setStage("done");
    } catch (err) {
      addLog("Geo/trust step failed: " + err.message, "err");
      setStage("geo");
      setProgress(0);
    }
  };
  ...
  useEffect(() => {
    if (stage === "done" && trustResult && profileData && walletData) {
  ...
  }, [stage, trustResult, profileData, walletData, sessionToken, dnaHash, geoData, onVerified]);
